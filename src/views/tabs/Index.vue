...
export default {
  ...
  methods: {
    ...
    getTabByTarget(target) {
      let res = null;
      if (target) {
        let box = util.jquery(target).closest(".tab-header");
        if (box.length > 0) {
    ...
    initHeader() {
      ...
      util.jquery(this.$refs.headerBox).scrollLeft(scrollLeft);

      offsetRightDistance = this.tabsBoxHeaderWidth - this.tabHeaderWidthCount;
      this.onOffsetRightDistance &&
        this.onOffsetRightDistance(offsetRightDistance);
    },
    initTabHeaderWidth() {
      this.tabsBoxHeaderWidth = util.jquery(this.$refs.tabsBoxHeader).width();
      this.headerBoxWidth = util.jquery(this.$refs.headerBox).width();
      ...
      this.tabs.forEach((one) => {
        if (one.headerWidth == null) {
          ...
          one.headerWidth = util.jquery(el).width();
        }
